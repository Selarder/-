...
import { BadRequestError, NotFoundError } from '../utils/custom-error.util';
...
export const startInstance = async (
  ...
  if (!definition) {
    throw new NotFoundError('流程定义不存在');
  }

  if (definition.status !== 'published') {
    throw new BadRequestError('流程定义未发布');
  }
...
export const getInstance = async (
  ...
  if (!instance) {
    throw new NotFoundError('流程实例不存在');
  }
...
export const cancelInstance = async (id: string): Promise<void> => {
  ...
  if (instance.status !== 'running') {
    throw new BadRequestError('流程实例未运行中');
  }
...
const executeProcess = async (
  ...
  if (!startNode) {
    throw new BadRequestError('流程定义缺少开始节点');
  }
